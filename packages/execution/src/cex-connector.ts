  ...
  const execute = async (adapter: CEXAdapter, plan: ExecutionPlan, side: 'buy' | 'sell'): Promise<LegResult> => {
    ...
    while (last.status === 'open' || last.status === 'pending' || last.status === 'partially_filled') {
      if (Date.now() - started >= config.maxUnhedgedMs) {
        ...
        const timedOut = await adapter.orderStatus(created.id, input.symbol).catch(() => last);
        await adapter.reconcile(created.id, input.symbol).catch(() => undefined);
        return {
          status: 'TIMEOUT',
          filled: Number(timedOut.filled || 0),
          average: timedOut.average,
    ...
    const reconciled = await adapter.orderStatus(created.id, input.symbol).catch(() => last);
    await adapter.reconcile(created.id, input.symbol).catch(() => undefined);
    return {
