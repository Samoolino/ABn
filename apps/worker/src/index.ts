...
async function refreshControlAndOpportunity(){ if(!db||executing)return; controlStatus=(await db.query(`select status from system_health where component='trading_control' limit 1`)).rows[0]?.status||'STOPPED'; if(mode==='DRY_RUN'||requestedMode!=='LIVE'||controlStatus!=='ARMED'){if(requestedMode!=='LIVE')mode=requestedMode==='DRY_RUN'?'DRY_RUN':'STOPPED';return;} if(!startup.allowed)return; if(!signerReady()){mode='DRY_RUN';return;} const candidate=await db.query(`select * from opportunities where status in ('EXECUTABLE','EXECUTABLE_NOW') and expires_at>now() and expected_net_profit>$1 order by expected_net_profit/greatest(capital_required,0.00000001) desc limit 1`,[safetyReserve]); const row=candidate.rows[0] as Record<string,unknown>|undefined;if(!row)return; const opportunity=mapOpportunity(row); if(opportunity.capitalSource!=='FUNDED_INVENTORY'||!Number.isFinite(opportunity.capitalRequired)||opportunity.capitalRequired<=0||opportunity.capitalRequired>policy.maxWorkingUsd)return; if(!isCexVenue(opportunity.buyVenue)||!isCexVenue(opportunity.sellVenue))return; let buyAdapter:CEXAdapter;let sellAdapter:CEXAdapter; try{buyAdapter=createCEXAdapterFromEnv(opportunity.buyVenue);sellAdapter=createCEXAdapterFromEnv(opportunity.sellVenue);await Promise.all([buyAdapter.connect(),sellAdapter.connect()]);await verifyCexInventory(buyAdapter,sellAdapter,opportunity);await resolveAndVerifySigner(opportunity.capitalRequired);}catch(error){mode='ARMED';console.error(JSON.stringify({event:'capital_or_signer_preflight_rejected',opportunityId:opportunity.id,error:String(error)}));return;} if(!executionEnabled)return; executing=true;mode='EXECUTING';const correlationId=crypto.randomUUID(); try{const connector=createCEXPairExecutionConnector(buyAdapter,sellAdapter);const result=await executePair(opportunity,{correlationId,opportunityId:opportunity.id,buy:{symbol:opportunity.symbol,amount:opportunity.quantity,type:'market'},sell:{symbol:opportunity.symbol,amount:opportunity.quantity,type:'market'},capital:{available:opportunity.capitalRequired,source:opportunity.capitalSource,commitmentMs:1500,repayable:false,repaymentAmount:0,collateralRequired:0}},connector,1500);mode=result.status==='COMPLETED'?'ARMED':'EMERGENCY_STOP';}catch(error){mode='EMERGENCY_STOP';console.error(JSON.stringify({event:'execution_error',correlationId,opportunityId:opportunity.id,error:String(error)}));}finally{executing=false;} }

console.log(JSON.stringify({event:'worker_start',mode,requestedMode,executionEnabled,capitalAsset,signerNetwork,startupGate:startup}));
