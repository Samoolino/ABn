...
async function refreshControlAndOpportunity(){ if(!db||executing)return; controlStatus=(await db.query(`select status from system_health where component='trading_control' limit 1`)).rows[0]?.status||'STOPPED'; if(mode==='DRY_RUN'||requestedMode!=='LIVE'||controlStatus!=='ARMED'){if(requestedMode!=='LIVE')mode=requestedMode==='DRY_RUN'?'DRY_RUN':'STOPPED';return;} if(!startup.allowed)return; if(!signerReady()||!hummingbotConfigured){mode='DRY_RUN';return;}
  // Opportunity-driven selection: there is no fixed dollar trigger. Select any fresh
  // opportunity whose authoritative expected net profit is strictly positive; capital,
  // liquidity, venue limits and safety reserve are validated after sizing.
  const candidate=await db.query(`select * from opportunities where status in ('EXECUTABLE','EXECUTABLE_NOW') and expires_at>now() and expected_net_profit>0 order by expected_net_profit/greatest(capital_required,0.00000001) desc limit 1`);
  ...
  if(!executionEnabled)return; executing=true;mode='EXECUTING'; const correlationId=opportunity.correlationId || crypto.randomUUID(); try{
