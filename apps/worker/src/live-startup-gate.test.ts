...
describe('validateLiveStartup', () => {
  it('allows non-LIVE modes without signer or RPC requirements', () => {
    ...
    expect(validateLiveStartup({ mode: 'DRY_RUN', requiredNetworks: [], registry, signerConfigured: false, executionEnabled: false })).toEqual({ allowed: true, mode: 'DRY_RUN', reasons: [] });
  });
  ...
  it('fails closed when LIVE execution is disabled', () => {
    ...
    const result = validateLiveStartup({ mode: 'LIVE', requiredNetworks: networks.map(n => n.network), registry, signerConfigured: true, executionEnabled: false });
  ...
  it('fails closed on a missing RPC', () => {
    ...
    const result = validateLiveStartup({ mode: 'LIVE', requiredNetworks: missing.map(n => n.network), registry, signerConfigured: true, executionEnabled: true });
    expect(result.allowed).toBe(false);
  ...
  it('fails closed on chain-id mismatch', () => {
    const registry = buildRegistryFromEnvironment({ requiredNetworks: networks, tokens: [] });
    const result = validateLiveStartup({ mode: 'LIVE', requiredNetworks: networks.map(n => n.network), registry, expectedChainIds: { ethereum: 1, arbitrum: 42161, base: 1, polygon: 137, bsc: 56 }, signerConfigured: true, executionEnabled: true });
    expect(result.allowed).toBe(false);
  ...
  it('allows LIVE only when every required runtime check passes', () => {
    const registry = buildRegistryFromEnvironment({ requiredNetworks: networks, tokens: [] });
    const result = validateLiveStartup({ mode: 'LIVE', requiredNetworks: networks.map(n => n.network), expectedChainIds: Object.fromEntries(networks.map(n => [n.network, n.chainId])), registry, signerConfigured: true, executionEnabled: true });
