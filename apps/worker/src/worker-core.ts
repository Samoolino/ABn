...
async function refreshControlAndOpportunity() {
  ...
  await db.query(`insert into opportunity_events(opportunity_id,event,details) values($1,'EXECUTION_GATE_READY',$2::jsonb)`, [
    opportunity.id,
    JSON.stringify({symbol:opportunity.symbol, netProfit:opportunity.netProfit, capitalRequired:opportunity.capitalRequired, reserve:safetyReserve}),
  ]);
  ...
  if (!isCexVenue(opportunity.buyVenue) || !isCexVenue(opportunity.sellVenue)) {
    ...
    const validation = validateCoordinatedOpportunity(opportunity, {
      correlationId,
      maxUnhedgedMs,
      minNetProfit: safetyReserve,
      maxQuoteAgeMs,
      isCexVenue,
    });
    mode = 'ARMED_EXECUTION_READY';
    await db.query(`insert into opportunity_events(opportunity_id,event,details) values($1,$2,$3::jsonb)`, [
      opportunity.id,
      validation.accepted ? 'COORDINATED_PLAN_VALIDATED' : 'COORDINATED_PLAN_REJECTED',
      JSON.stringify({correlationId,reason:validation.reason,plan:validation.plan,executionReleased:false}),
    ]);
...
async function heartbeat() {
  ...
  if (db) await db.query(`insert into system_health(component,status,heartbeat_at,details) values($1,$2,now(),$3::jsonb) on conflict(component) do update set status=excluded.status,heartbeat_at=excluded.heartbeat_at,details=excluded.details`, ['worker',mode,JSON.stringify({mode,requestedMode,controlStatus,signer:signerVerification?'VERIFIED':signerReady()?'CONFIGURED':'NOT_CONFIGURED',executionEnabled,capitalSource:requiredCapitalSource,signerNetwork,capitalAsset,maxQuoteAgeMs})]);
